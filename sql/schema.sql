...
CREATE TABLE IF NOT EXISTS `ic_pagos_confirmados` (
  ...
  `solicitud_baja` TINYINT(1) DEFAULT 0,
  `motivo_baja` TEXT NULL,
  -- Snapshot al momento de la aprobación (inmutable ante cambios posteriores en créditos)
  `es_cuota_pura`    TINYINT(1) NOT NULL DEFAULT 0     COMMENT 'Cobrador pagó solo capital, mora pendiente',
  `observaciones`    TEXT NULL                          COMMENT 'Observaciones del cobrador',
  `fecha_jornada`    DATE NULL                          COMMENT 'Fecha de jornada original del cobrador',
  `origen`           VARCHAR(20) NOT NULL DEFAULT 'cobrador' COMMENT 'cobrador | manual',
  `monto_cuota_orig` DECIMAL(12,2) NULL                COMMENT 'Snapshot monto cuota al aprobar',
...
CREATE TABLE IF NOT EXISTS `ic_historial_refinanciaciones` (
  ...
  `frecuencia_nueva`     ENUM('diario','semanal','quincenal','mensual') NOT NULL,
  `observaciones`        TEXT NULL,
  FOREIGN KEY (`credito_id`) REFERENCES `ic_creditos`(`id`) ON DELETE CASCADE,
  FOREIGN KEY (`usuario_id`) REFERENCES `ic_usuarios`(`id`),
