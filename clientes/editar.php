...
$zona_por_usuario = ['santizalazar' => 'Zona 1', 'jpbicego' => 'Norte', 'enzoteceira' => 'Sur', 'sebadelga' => 'Este', 'masanchez' => 'Zona 4-6', 'TURCOCOBRANZA' => 'Sur', 'davbrizuela' => 'Norte'];
$cob_zona_map = [];
...
$cob_dias_map = [];
foreach ($cobradores as $cb) { $cob_dias_map[(int)$cb['id']] = in_array($cb['usuario'], $USUARIOS_SEMANA_COMPLETA, true); }
$dia_cobro_semana_completa = $cob_dias_map[(int)($c['cobrador_id'] ?? 0)] ?? false;
    ...
    if (!$error) {
        $pdo->prepare("
            UPDATE ic_clientes SET
              ...
              fecha_nacimiento=?, direccion=?, localidad=?, barrio=?, calle1=?, calle2=?, direccion_laboral=?, coordenadas=?,
        ...
        // Sincronizar cobrador en créditos activos (siempre, no solo si cambió el campo del
        // cliente — evita que un crédito que divergió por otro motivo quede así para siempre)
