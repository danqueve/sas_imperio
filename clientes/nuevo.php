...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        ...
        if (!empty($v['tiene_garante']) && !empty($v['g_nombres']) && !empty($v['g_apellidos'])) {
            ...
            ")->execute([
                        $cliente_id,
                        sanear_csv_formula($v['g_nombres']),
                        sanear_csv_formula($v['g_apellidos']),
                        trim($v['g_dni'] ?? ''),
                        trim($v['g_cuil'] ?? ''),
                        sanear_csv_formula($v['g_telefono'] ?? ''),
                        sanear_csv_formula($v['g_direccion'] ?? ''),
                        sanear_csv_formula($v['g_localidad'] ?? ''),
